...
const {initDatabase, getConnection} = require('./db.js');
const fs = require('fs');

var indexRouter = require('./routes/index');
var dogsRouter = require('./routes/dogs');
var walkRequestRouter = require('./routes/walkRequest');
...
(async () => {
  try {
    ...
    try{
        ...
        const [req_count] = await db.execute('SELECT COUNT(*) AS count FROM WalkRequests');
        if (req_count[0].count === 0) {
          await db.query(`
            ...
            INSERT INTO WalkRequests(dog_id, requested_time, duration_minutes, location, status)
            VALUES ((SELECT dog_id FROM Dogs WHERE name = 'Jimmy'), '2025-06-10 10:30:00', 30, 'Northfield', 'completed');

            INSERT INTO WalkRequests(dog_id, requested_time, duration_minutes, location, status)
            VALUES ((SELECT dog_id FROM Dogs WHERE name = 'Jimmy'), '2025-06-11 10:00:00', 60, 'Parafield', 'open');

            INSERT INTO WalkRequests(dog_id, requested_time, duration_minutes, location, status)
            VALUES ((SELECT dog_id FROM Dogs WHERE name = 'Tommy'), '2025-06-11 10:00:00', 60, 'Parafield', 'open');
          `);
        }
    }
    catch(err){
...
app.use('/api/walkrequests/:status', walkRequestRouter);
